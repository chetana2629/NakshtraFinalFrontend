...
export const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:7350/user/login', { username, password, role });
    
            console.log('API Response:', response.data); // Ensure this logs the expected role
            ...
            if (response.status === 200) {
                ...
                switch (userRole) {
                    ...
                    case 'designer':
                        ...
                        break;
                    default:
                        setError(`Invalid role: ${userRole}.`); // Provide specific role in error message
                        break;
                }
            } else {
